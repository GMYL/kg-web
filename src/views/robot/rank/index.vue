            ...
            <Card>
                ...
                <div style="padding-bottom: 200px;">
                    <Table highlight-row border ref="selection" :columns="mainColumns" :data="mainData"
                           @on-selection-change="selectAry" ellipsis highlight-row></Table>
                    <br/>
                ...
                <Modal v-model="modal3" title="修改" @on-ok="updateEditor">
                    <Form :model="editmainData" label-position="left" :label-width="100">
                        <FormItem label="(必填) 扩展词">
                            <Input v-model="editmainData.word" placeholder="填写扩展词"/>
                        </FormItem>
                        <FormItem label="(必填) 权重">
                            <Input v-model="editmainData.weight" placeholder="填写权重"/>
                        </FormItem>
                        <FormItem label="类型">
...
<script>
  ...
  export default {
    ...
    methods: {
      getData() {
        this.$http.post('/dict/getWordRankingByOccur', this.$data.filterData).then(res => {
      ...
      searchClick() {
        ...
        this.$http.get("/dict/getWordRankingByOccur/" + obj).then((res) => {
          this.mainData = res.list.map((val) => {
            return val;
          });
          this.filterData.page.totalSize = res.totalSize;
