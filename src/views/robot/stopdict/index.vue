            ...
            <Card>
                <Row slot="title">
                    <i-col span="2" style="height: 24px; padding-top: 10px;">
                        <Icon type="navicon-round"></Icon> 停用词词典
                    </i-col>
                    <i-col span="10">
                        <!-- <Select v-model="filterData.excelType" @on-change="freshData" style="width:220px" clearable placeholder="请选择">
                            <Option value="问答">问答</Option>
                            <Option value="法规" >法规</Option>
                            <Option value="文章">文章</Option>
                        </Select> -->
                        <Input type="text" v-model="filterData.word" style="width: 250px" @on-enter="freshData" clearable placeholder="请输入停用词"/>
                ...
                <Button slot="extra" type="primary" shape="circle" @click="handleSelectAll(true)">设置全选</Button>
                <Button slot="extra" type="primary" shape="circle" @click="handleSelectAll(false)">取消全选</Button>
                <Button v-show="expertIdAry.length > 0" slot="extra" type="error" shape="circle" @click="delSum">
                    <Icon type="trash-a"></Icon>
                    批量删除
                </Button>
                <!-- <Button slot="extra" type="error" shape="circle" @click="modal4 = true">
                    <Icon type="trash-a"></Icon>
                    按照分类全部删除
                </Button> -->
                ...
                <div style="padding-bottom: 200px;">
                    <Table highlight-row border ref="selection" :columns="mainColumns" :data="mainData" @on-selection-change="selectAry" ellipsis highlight-row></Table>
                    <br/>
                    <Page v-show="filterData.page.totalSize > 10" :total="filterData.page.totalSize" @on-change="getPageData"
                          @on-page-size-change="setPageSize" :current="filterData.page.pageNumber" class="pageTool" show-sizer :page-size="20" :page-size-opts="[10,20,50,100,200,500,1000]"></Page>
                </div>
                <Modal v-model="modal1" title="停用词词典详情">
                    <!-- <p v-show="dataDetal.excelType"><Tag class="fooo">类型</Tag><span v-text="dataDetal.excelType"></span></p> -->
                    <p v-show="dataDetal.word"><Tag class="fooo">停用词</Tag><span v-text="dataDetal.word"></span></p>
                    <p v-show="dataDetal.createTime"><Tag class="fooo">创建时间</Tag><span v-text="dataDetal.dateTime"></span></p>
                    <p v-show="dataDetal.updateTime"><Tag class="fooo">修改时间</Tag><span v-text="dataDetal.dateUpdateTime"></span></p>
                </Modal>
                <Modal v-model="modal2" title="添加" @on-ok="addData">
                    <Form :model="addItemData" label-position="left" :label-width="100">
                        <FormItem label="(必填) 停用词">
                            <Input v-model="addItemData.word" placeholder="填写词"></Input>
                        </FormItem>
                        <!-- <FormItem label="类型"> -->
                ...
                <Modal v-model="modal3" title="修改" @on-ok="updateEditor">
                    <Form :model="editmainData" label-position="left" :label-width="100">
                        <FormItem label="(必填) 停用词">
                            <Input v-model="editmainData.word" placeholder="填写停用词"></Input>
                        </FormItem>
                        <!-- <FormItem label="类型">
                ...
                <Modal v-model="deleteConfirm" title="提示" @on-ok="deleteOk" @on-cancel="cancelDelete">
                    <p>是否确认删除<Tag>{{editmainData.word}}</Tag>？</p>
                </Modal>
            </Card>
        </i-col>
    </Row>
...
<script>
  ...
  export default {
    ...
    methods: {
      getData () {
        this.$http.post('/stopdict/query', this.$data.filterData).then(res => {
      ...
      deleteEditor (rowData) {
        this.$http.delete('/stopdict/' + rowData.id).then(() => {
          this.$Notice.success({
            title: '删除成功'
          });
          this.getData();
      ...
      delSum () {
        //  批量删除
        this.$http.delete('/stopdict/all', {data: this.expertIdAry}).then(() => {
          this.$Notice.success({
            title: '删除成功'
          });
          this.getData();
      ...
      updateEditor () {
        this.$http.post('/stopdict/update', this.$data.editmainData).then(() => {
          this.$Notice.success({
            title: '修改成功'
          });
        });
      ...
      addData () {
        ...
        this.$http.put('/stopdict', obj).then(() => {
          this.$Notice.success({
            title: '添加成功'
          });
          //  成功后数据置为空
        ...
        this.$http.delete('/stopdict/deltype/'+ obj.excelType).then(() => {
          this.$Notice.success({
            title: '添加成功'
          });
          //  成功后数据置为空
      ...
      freshData () {
        ...
        deleteOk () {
            this.$http.delete('/stopdict/' + this.deleteParems).then(() => {
                this.$Notice.success({
                    title: '删除成功',
                });
                this.freshData();
      ...
      searchClick() {
        ...
        this.$http.get("/stopdict/query/" + obj).then((res) => {
            this.mainData = res.list.map((val) => {
                return val;
            });
            this.filterData.page.totalSize = res.totalSize;
