  ...
  <div>
    <Row>
      <i-col span="24" style="padding-right: 12px;">
        <Card>
          <p slot="title">
            <Icon type="ios-list-outline"></Icon>注册用户列表
          </p>
          ...
          <div class="filterTool">
            <div class="toolBar">
              ...
              <Button type="text" @click="advancedQueryModal = !advancedQueryModal">
                <Icon type="ios-settings-strong"></Icon>&nbsp;高级查询
              </Button>
            </div>
          </div>
          <Card :bordered="true" style="margin-bottom: 10px;width:950px" v-if="advancedQueryModal">
            <Form label-position="right" :label-width="60" inline>
              <FormItem label="用户名">
                <Input v-model="filterData.userName" placeholder="填写合作人姓名"></Input>
              </FormItem>
              <FormItem label="真实姓名">
                <Input v-model="filterData.trueName" placeholder="填写合作人姓名"></Input>
              </FormItem>
              <FormItem label="公司">
                <Input v-model="filterData.company" placeholder="填写所属公司"></Input>
              </FormItem>
              <FormItem>
                <Button type="primary" @click="getData">查询</Button>
                <Button
                  type="dashed"
                  @click="filterData.userName = '';filterData.company = '';filterData.trueName = ''"
                >清空</Button>
              </FormItem>
            </Form>
          </Card>
...
export default {
  ...
  data() {
    return {
      ...
      advancedQueryModal: false, // 高级查询框
      groupDate: [],
      rolesDate: [],
      filterData: {
        valid: true,
        group: ["expert","vip","internalStaff","sim"],
        page: {
          pageNumber: 1,
          totalSize: 10,
          pageSize: 10
  ...
  methods: {
    ...
    getData() {
      this.$http.post("/user/edit/normalquery", this.filterData).then(res => {
